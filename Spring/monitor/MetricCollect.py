...
def collect_call_latency(config: Config, _dir: str):
    ...
    # P50，P90，P99
    prom_50_sql = (
        'histogram_quantile(0.50, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload_namespace="%s"}[1m])) by (destination_workload, destination_workload_namespace, source_workload, le))'
        % config.namespace
    )
    prom_90_sql = (
    ...
    prom_99_sql = (
        'histogram_quantile(0.99, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload_namespace="%s"}[1m])) by (destination_workload, destination_workload_namespace, source_workload, le))'
        % config.namespace
    )
...
# Get the response time for every service
def collect_svc_latency(config: Config, _dir: str):
    ...
    prom_99_sql = (
        'histogram_quantile(0.99, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload_namespace="%s"}[1m])) by (destination_workload, destination_workload_namespace, le))'
        % config.namespace
    )
    responses_50 = prom_util.execute_prom(config.prom_range_url, prom_50_sql)
    responses_90 = prom_util.execute_prom(config.prom_range_url, prom_90_sql)
...
def collect_resource_metric(config: Config, _dir: str):
    ...
    vCPU_sql = (
        'sum(rate(container_cpu_usage_seconds_total{namespace="%s"}[1m]))'
        % config.namespace
    )
    mem_sql = (
        'sum(rate(container_memory_usage_bytes{namespace="%s"}[1m])) / (1024*1024)'
        % config.namespace
    )
    prom_util = PrometheusClient(config)
...
def collect_pod_num(config: Config, _dir: str):
    ...
    qps_sql = (
        'count(kube_pod_info{namespace="%s"}) by (created_by_name)' % config.namespace
    )
    ...
    def handle(result, instance_df):
        if "created_by_name" in result["metric"]:
            ...
            if "timestamp" not in instance_df:
                timestamp = values[0]
                instance_df["timestamp"] = timestamp
                instance_df["timestamp"] = instance_df["timestamp"].astype(
                    "datetime64[s]"
                )
            metric = values[1]
            instance_df[name] = pd.Series(metric)
...
def collect_svc_qps(config: Config, _dir: str):
    ...
    def handle(result, qps_df):
        name = result["metric"]["destination_workload"] + "&qps"
        values = result["values"]
        if values == []:
            return
        values = list(zip(*values))
...
# get qps for microservice
def collect_svc_qps_source(config: Config, _dir: str):
    qps_df = pd.DataFrame()
    prom_util = PrometheusClient(config)
    qps_sql = (
    ...
    def handle(result, qps_df):
        name = result["metric"]["source_workload"] + "&qps"
        values = result["values"]
        if values == []:
            return
        values = list(zip(*values))
    ...
    path = os.path.join(_dir, "svc_qps_source.csv")
    qps_df.to_csv(path, mode="a", header=False, index=False)
...
def collect_svc_metric(config: Config, _dir: str):
    ...
    final_df.to_csv(_dir + "/svc_metric.csv", mode="a", header=False, index=False)
...
def collect_succeess_rate(config: Config, _dir: str):
    ...
    def handle(result, success_df):
        name = result["metric"]["destination_workload"] + "&succ"
        values = result["values"]
        if values == []:
            return
        values = list(zip(*values))
