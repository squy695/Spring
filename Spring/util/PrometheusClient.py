...
class PrometheusClient:
    ...
    def p90(self, svc):
        prom_90_sql = (
            'histogram_quantile(0.90, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload="%s"}[1m])) by (destination_workload, le))'
            % svc
        )
    ...
    def p50(self, svc):
        prom_50_sql = (
            'histogram_quantile(0.50, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload="%s"}[1m])) by (destination_workload, le))'
            % svc
        )
    ...
    def get_call(self):
        ...
        svcs = [
            svc
            for svc in k8s_util.get_svcs()
            if "redis" not in svc
            and "mongo" not in svc
            and "unknown" not in svc
            and "mysql" not in svc
            and "rabbitmq" not in svc
        ]
        DG.add_nodes_from(svcs)
        edges = []
        prom_sql = (
            'sum(istio_tcp_received_bytes_total{destination_workload_namespace="%s"}) by (source_workload, destination_workload)'
            % self.namespace
        )
        results = self.execute_prom(self.prom_no_range_url, prom_sql)

        prom_sql = (
            'sum(istio_requests_total{destination_workload_namespace="%s"}) by (source_workload, destination_workload)'
            % self.namespace
        )
    ...
    def get_call_latency(self):
        ...
        prom_90_sql = (
            'histogram_quantile(0.90, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload_namespace="%s"}[1m])) by (destination_workload, source_workload, le))'
            % self.namespace
        )
        ...
        if len(responses_90) > 0:
            for res in responses_90:
                name = (
                    res["metric"]["source_workload"]
                    + "_"
                    + res["metric"]["destination_workload"]
                )
    ...
    def get_svc_latency(self):
        ...
        prom_url = self.prom_no_range_url
        prom_50_sql = (
            'histogram_quantile(0.50, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload_namespace="%s"}[1m])) by (destination_workload, le))'
            % self.namespace
        )
        prom_90_sql = (
            'histogram_quantile(0.90, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload_namespace="%s"}[1m])) by (destination_workload, le))'
            % self.namespace
        )
        prom_99_sql = (
            'histogram_quantile(0.99, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload_namespace="%s"}[1m])) by (destination_workload, le))'
            % self.namespace
        )
    ...
    def get_svc_qps(self):
        qps_dic = {}
        qps_sql = (
            'sum(rate(istio_requests_total{reporter="destination",namespace="%s"}[1m])) by (destination_workload)'
            % self.namespace
        )
    ...
    # 获取每个调用的请求返回时间的 P90 百分位数，返回的是表，也就是在range区间内测量多测得到的p90
    def get_call_p90_latency_range(self):
        call_df = pd.DataFrame()
        prom_90_sql = (
            'histogram_quantile(0.90, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload_namespace="%s"}[1m])) by (destination_workload, destination_workload_namespace, source_workload, le))'
            % self.namespace
        )
    ...
    # 获取每个服务的端到端返回时间的 P90 百分位数，返回的是表，也就是在range区间内测量多测得到的p90
    def get_svc_p90_latency_range(self):
        latency_df = pd.DataFrame()
        prom_90_sql = (
            'histogram_quantile(0.90, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload_namespace="%s"}[1m])) by (destination_workload, destination_workload_namespace, le))'
            % self.namespace
        )
        responses_90 = self.execute_prom(self.prom_range_url, prom_90_sql)
        ...
        def handle(result, latency_df):
            ...
            if "timestamp" not in latency_df:
                ...
                latency_df["timestamp"] = latency_df["timestamp"].astype(
                    "datetime64[s]"
                )
            metric = values[1]
            key = name
    ...
    # 获取每个服务的端到端返回时间的 P50 百分位数，返回的是表，也就是在range区间内测量多测得到的p50
    def get_svc_p50_latency_range(self):
        latency_df = pd.DataFrame()
        prom_90_sql = (
            'histogram_quantile(0.50, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload_namespace="%s"}[1m])) by (destination_workload, destination_workload_namespace, le))'
            % self.namespace
        )
        responses_90 = self.execute_prom(self.prom_range_url, prom_90_sql)
    ...
    def get_resource_metric_range(self):
        ...
        vCPU_sql = (
            'sum(rate(container_cpu_usage_seconds_total{job="kubernetes-cadvisor",container!~\'POD|istio-proxy|\',namespace="%s"}[1m]))'
            % self.namespace
        )
        mem_sql = (
            'sum(rate(container_memory_usage_bytes{job="kubernetes-cadvisor",container!~\'POD|istio-proxy|\', namespace="%s"}[1m])) / (1024*1024)'
            % self.namespace
        )
    ...
    # Get qps for microservices，是表
    def get_svc_qps_range_source(self):
        qps_df = pd.DataFrame()
        qps_sql = (
            'sum(rate(istio_requests_total{reporter="source",namespace="%s"}[1m])) by (source_workload)'
            % self.namespace
        )
        response = self.execute_prom(self.prom_range_url, qps_sql)
    ...
    def get_svc_metric_range(self):
        ...
        k8s_util = KubernetesClient(self.config)
        cpu_usage_sql = (
            "(sum(rate(container_cpu_usage_seconds_total{namespace=\"%s\",container!='POD|istio-proxy|'}[1m])) by(pod))"
            % (self.namespace)
        )
        cpu_limit_sql = (
            "(sum(container_spec_cpu_quota{namespace=\"%s\",container!='POD|istio-proxy|'}) by(pod) /100000)"
            % (self.namespace)
        )
        # memory usage MB
        mem_usage_rate_sql = (
            "sum(rate(container_memory_usage_bytes{namespace=\"%s\",container!='POD|istio-proxy|'}[1m])) by(pod) / (1024*1024)"
            % (self.namespace)
        )
        mem_usage_sql = (
            "sum(container_memory_usage_bytes{namespace=\"%s\",container!='POD|istio-proxy|'}) by(pod) / (1024*1024)"
            % (self.namespace)
        )
        mem_limit_sql = (
            "sum(container_spec_memory_limit_bytes{namespace=\"%s\",container_name!='POD|istio-proxy|'}) by(pod) / (1024*1024)"
            % (self.namespace)
        )
        # file IO usage MB
        fs_usage_sql = (
            "(sum(rate(container_fs_usage_bytes{namespace=\"%s\",container!='POD|istio-proxy|'}[1m])) by(pod)) / (1024*1024)"
            % (self.namespace)
        )
        fs_write_sql = (
            "(sum(rate(container_fs_write_seconds_total{namespace=\"%s\",container!='POD|istio-proxy|'}[1m])) by(pod))"
            % (self.namespace)
        )
        fs_read_sql = (
            "(sum(rate(container_fs_read_seconds_total{namespace=\"%s\",container!='POD|istio-proxy|'}[1m])) by(pod))"
            % (self.namespace)
        )
        # network KB/s
        net_receive_sql = (
            'sum(rate(container_network_receive_bytes_total{namespace="%s"}[1m])) by (pod) / 1024'
            % (self.namespace)
        )
        net_trainsmit_sql = (
            'sum(rate(container_network_transmit_bytes_total{namespace="%s"}[1m])) by (pod) / 1024'
            % (self.namespace)
        )
        ...
        for svc in svcs:
            ...
            svc_cols = [
                col
                for col in col_list
                if col.startswith(svc) and col.endswith("cpu_usage")
            ]
            ...
            svc_cols = [
                col
                for col in col_list
                if col.startswith(svc) and col.endswith("cpu_limit")
            ]
            final_df[svc + "&cpu_limit"] = df[svc_cols].sum(axis=1)
            svc_cols = [
                col
                for col in col_list
                if col.startswith(svc) and col.endswith("mem_usage")
            ]
            final_df[svc + "&mem_usage"] = df[svc_cols].sum(axis=1)
            svc_cols = [
                col
                for col in col_list
                if col.startswith(svc) and col.endswith("mem_usage_rate")
            ]
            final_df[svc + "&mem_usage_rate"] = df[svc_cols].sum(axis=1)
            svc_cols = [
                col
                for col in col_list
                if col.startswith(svc) and col.endswith("mem_limit")
            ]
            final_df[svc + "&mem_limit"] = df[svc_cols].sum(axis=1)
            svc_cols = [
                col
                for col in col_list
                if col.startswith(svc) and col.endswith("fs_usage")
            ]
            final_df[svc + "&fs_usage"] = df[svc_cols].sum(axis=1)
            svc_cols = [
                col
                for col in col_list
                if col.startswith(svc) and col.endswith("fs_write")
            ]
            final_df[svc + "&fs_write"] = df[svc_cols].sum(axis=1)
            svc_cols = [
                col
                for col in col_list
                if col.startswith(svc) and col.endswith("fs_read")
            ]
            final_df[svc + "&fs_read"] = df[svc_cols].sum(axis=1)
            svc_cols = [
                col
                for col in col_list
                if col.startswith(svc) and col.endswith("net_receive")
            ]
            final_df[svc + "&net_receive"] = df[svc_cols].sum(axis=1)
            svc_cols = [
                col
                for col in col_list
                if col.startswith(svc) and col.endswith("net_trainsmit")
            ]
            final_df[svc + "&net_trainsmit"] = df[svc_cols].sum(axis=1)

            # final_df = pd.concat([final_df, df[svc_cols].sum(axis=1)], axis=1)
    ...
    def get_success_rate_range(self):
        ...
        success_rate_sql = (
            '(sum(rate(istio_requests_total{reporter="destination", response_code!~"5.*",namespace="%s"}[1m])) by (destination_workload, destination_workload_namespace) / sum(rate(istio_requests_total{reporter="destination",namespace="%s"}[1m])) by (destination_workload, destination_workload_namespace))'
        ...
        def handle(result, success_df):
            ...
            if "timestamp" not in success_df:
                ...
                success_df["timestamp"] = success_df["timestamp"].astype(
                    "datetime64[s]"
                )
            metric = values[1]
    ...
    # 过去一分钟所有服务的请求和响应消息数，应该是range再求和，才能表示过去一分钟内的请求滞留率
    def get_svc_in_out_count(self) -> pd.DataFrame:
        df = pd.DataFrame()
        all_qps_sql = (
            'sum(rate(istio_requests_total{reporter="destination",namespace="%s"}[1m])) by (destination_workload, namespace, source_workload)'
            % self.namespace
        )

        response = self.execute_prom(self.prom_range_url, all_qps_sql)
        ...
        def handle(result, df):
            ...
            qps = values[1]
            key = name
            df[key] = pd.Series(qps)
            df[key] = df[key].astype("float64")

        [handle(result, df) for result in response]
